...
func updateUserInfoHandler(formatter *render.Render) http.HandlerFunc {
	...
	return func(w http.ResponseWriter, req *http.Request) {
		...
		if len(req.Form["userid"][0]) != 0 {
			...
			user := entities.UserInfo{UID: int(i), UserName: username, DepartName: departname, CreateAt: created}

			err := entities.UserInfoService.UpdateByID(int(i), username, departname, created)
